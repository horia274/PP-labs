...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ([segment (get-line-segment)]
         [p1 (get-start-point segment)]
         [p2 (get-end-point segment)]
         [x1 (car p1)]
         [y1 (cdr p1)]
         [x2 (car p2)]
         [y2 (cdr p2)])
...
(define (compute-perimeter points)
  (let helper ([first-point (car points)] [points points])
    (if (null? (cdr points))
        (distance (car points) first-point)
        (+ (distance (car points) (cadr points)) (helper first-point (cdr points))))))
...
(define (3-sequence-max numbers separator)
  ...
  (let*-values ([(l1 next-l1) (splitf-at numbers pred)]
                [(l2 next-l2) (splitf-at (cdr next-l1) pred)]
                [(l3) (cdr next-l2)])
    (max (sum l1) (sum l2) (sum l3)))))
...
(define (list-num-concat numbers)
  (foldl (λ (x acc) (string->number (string-append (number->string acc) (number->string x))))
         (car numbers)
         (cdr numbers)))
...
(define (find-all-suffixes number)
  (let helper ([digits (number->list number)]
    ...
    (if (null? digits)
        (reverse acc)
        (helper (cdr digits) (cons (list-num-concat digits) acc)))))
...
(define (run initial-state final-state next)
  (let helper ([current-state initial-state])
    (if (equal? current-state final-state)
        (list current-state)
        (cons current-state (helper (next current-state))))))
...
(define (generate-number k x)
  (let ([pred (λ (x) (+ x k))])
    (list-num-concat (run x (+ (* (sub1 k) k) x) pred))))
