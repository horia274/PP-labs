...
(define (all-palindromes? n Bases)
  (or (null? Bases) (and (palindrome? (num->base n (car Bases))) (all-palindromes? n (cdr Bases)))))
...
(define (first-b-pal start b)
  (if (check-min-pal start b '(2 3 4 5 6 7 8 9 10))
      start
      (first-b-pal (+ start 1) b)))

(define (check-min-pal n b Bases)
  (cond
    [(= b 0) #t]
    [(null? Bases) #f]
    [(palindrome? (num->base n (car Bases))) (check-min-pal n (- b 1) (cdr Bases))]
    [else (check-min-pal n b (cdr Bases))]))
...
(define (longest-palindrome n)
  (cond
    [(palindrome? (num->base n 10)) n]
    [(< (count-digits (longest-palindrome (take-first n))) (count-digits (longest-palindrome (take-last n)))) (longest-palindrome (take-last n))]
    [else (longest-palindrome (take-first n))]))
